...
#define CHECK_HRESULT(x) { HRESULT ckhr = ((x)); if (FAILED(ckhr)) throw CAtlException(ckhr); }
...
void VssCopy::WaitAndQueryStatus(ATL::CComPtr<IVssAsync> pVssAsync)
{
	CHECK_HRESULT( pVssAsync->Wait() );

	HRESULT hr; 
    CHECK_HRESULT( pVssAsync->QueryStatus(&hr, NULL) ); 
